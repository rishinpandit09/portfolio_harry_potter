...
export default function Projects() {
  return (
    <section id="projects" className="section">
      <h2 className="text-3xl font-lumos text-slytherin sparkle">
        Mischief Managed (Projects)
      </h2>
      <div className="space-y-8">
        {projects.map((p, i) => (
          <div key={i} className="card text-left">
            <a
              href={p.link}
              className="text-2xl font-lumos text-gryffindor hover:underline"
              target="_blank"
              rel="noopener noreferrer"
            >
              {p.title}
            </a>
            <p className="mt-1 text-sm text-ravenclaw">
              Tech: {p.tech.join(", ")}
            </p>
            <ul className="list-disc pl-5 mt-2 space-y-1 text-ravenclaw">
              {p.bullets.map((b, j) => (
                <li key={j}>{b}</li>
              ))}
            </ul>
